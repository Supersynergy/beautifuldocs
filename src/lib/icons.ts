...
// Fetches icons from unpkg CDN and embeds as SVG
...
// Popular icons pre-defined (to avoid network calls)
const POPULAR_ICONS: IconCache = {
...
export function processIcons(html: string): string {
  // Replace :icon-name: with SVG
  // Support sizes: :icon-name-sm:, :icon-name-lg:
  return html.replace(/:([a-z-]+)(?:-(sm|md|lg))?:/g, (match, name, size) => {
    const svg = getIconSvg(name);
    if (!svg) return match;
    
    const pixelSize = size === 'sm' ? 16 : size === 'lg' ? 32 : 24;
    return wrapIcon(svg, pixelSize);
  });
}
...
function getIconSvg(name: string): string | null {
  ...
  // Check popular icons
  if (POPULAR_ICONS[name]) {
    iconCache[name] = POPULAR_ICONS[name];
    return POPULAR_ICONS[name];
  }
  
  // Return null for unknown icons (could fetch from CDN in future)
  return null;
}

function wrapIcon(svg: string, size: number): string {
  // Update SVG size and add class
  return svg
